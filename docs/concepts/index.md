...
The first page most readers need is [the ledger](/concepts/ledger). It's where every fact goes, and it's why the second run of a pipeline is cheaper than the first. [Identity keys](/concepts/identity-keys), [canonical fields](/concepts/canonical-fields), and [provenance](/concepts/facts) are the three pages that follow it and that every guide depends on.

If you haven't run anything yet, [Start](/start) will get you a [receipt](/concepts/runs-and-receipts) in a few minutes, and the concepts read better with one in front of you.
